...
- `iso: Dict`:
  ...
  - `templates: List[str]` --- List of template names the ISO installs (example value: `[fedora-43-xfce, debian-13-xfce, whonix-gateway-18]`). They are grouped by family (the name prefix, e.g. `fedora`) into the comps groups the kickstart pulls (`@fedora`, `@debian`, `@whonix`). A kickstart group selection with no matching comps group (a family with no configured template) is dropped at parse time, so shrinking this list does not require editing the kickstart. The template packages are resolved at `prep` time from the `builder-local` repository, which contains templates built locally (found in `artifacts/templates`) and the installer templates cache, or downloaded from the template repositories the kickstart enables. This list is never downloaded automatically: to pre-fetch a template that is not built locally, list it in `cache:templates` too.
...
- `cache: Dict` --- List of distributions cache options.
  ...
  - `templates: List[str]` --- Template names to pre-download into the installer cache at `init-cache` time, from the template repositories the selected kickstart enables. The list is independent from `iso:templates`: neither list is added to the other. Cache the `iso:templates` you do not build locally so `prep` works offline, and add templates the ISO does not install if you want them around. Locally built templates need no caching, they are used straight from `artifacts/templates`. Leave the list empty for a kickstart that enables no template repository. `./qb cleanup --installer-templates-only-unused` removes cached templates outside this list, and `--installer-templates-only-old` keeps only the newest build of each name in it.
