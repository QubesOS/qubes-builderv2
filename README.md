...
It is the recommended starting point, and is the only option before Qubes Windows Tools are built.
...
The `tools/windows/` directory contains scripts that create and fully configure the worker qube.
...
- Creates a `StandaloneVM` HVM with a 40 GiB root volume.
...
- Shuts the worker down. The EWDK ISO is attached automatically by the executor at build time via `admin.vm.device.block.Attach`. It does not need to be attached manually.

After the script completes, the worker qube is ready. Configure the builder to use it:
      ...
      stages:
        - build:
            executor:
              type: windows-ssh
              options:
                ssh-vm: win-build             # qube to start automatically
...
2. Mount the ISO via PowerShell `Mount-DiskImage` (idempotent — skipped if already mounted).
...
It requires a Windows disposable template with Qubes Windows Tools already installed.
Use the `windows-ssh` executor to build QWT first, then create the disposable template from the resulting image.
      ...
      stages:
        - build:
            executor:
              type: windows
              options:
                dispvm: win-build    # disposable Windows template (default: win-build)
                ewdk: tools/windows/ewdk.iso
                user: user
```

The `qubesbuilder.WinFileCopyIn` and `qubesbuilder.WinFileCopyOut` RPC services (from `rpc/`) must be installed in the Windows disposable template and made persistent via [bind-dirs](https://www.qubes-os.org/doc/bind-dirs/).
...
sudo cp qubesbuilder.WinSign.{common,CreateKey,DeleteKey,GetCert,QueryKey,Sign} /etc/qubes-rpc/
```
...
sudo cp rpc/qubesbuilder.WinSign.Timestamp /etc/qubes-rpc/
```
...
```bash
sudo cp rpc/policy/51-qubesbuilder-windows.policy /etc/qubes/policy.d/
```

Review the policy file and adjust the qube names (`work-qubesos`, `vault-sign`, `win-build`) to match your setup before copying.
...
- Options specific to the `windows` and `windows-ssh` executors (see `example-configs/windows-tools.yml`):
  ...
  - `ewdk: str` --- Path to the EWDK iso file. With `ssh-vm`: attached as a block device to the worker qube. Without `ssh-vm`: SCPed to `c:\Users\<user>\ewdk.iso` on the remote host and mounted via PowerShell on the first run.
...
- Options specific to the `windows-ssh` executor:
  ...
  - `ewdk-skip-checksum: bool` --- When `ssh-vm` is not set, skip SHA256 verification and only transfer the EWDK ISO if the file is absent on the remote host (default: `false`). Use this to speed up runs when the remote machine is trusted.
...
Publishing is enabled as long as `repository-publish: components` is set in the builder configuration.
...
repository-publish/windows/<qubes-release>/<repository>/vm/<dist>/<component>_<version>/
    bin/        # compiled binaries (.exe, .dll, .sys, ...)
    inc/        # header files (.h)
    lib/        # import libraries (.lib)
    sign.crt    # Authenticode certificate (if present)
